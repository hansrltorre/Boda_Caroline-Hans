...
// =========================
// MODAL AGENDA (cargado dinámicamente desde modal-agenda.html)
// =========================
const openAgendaBtn = document.getElementById('openAgendaModalBtn');

function setupAgendaModalBindings() {
  const agendaModal = document.getElementById('agendaModal');
  const closeAgendaBtn = document.getElementById('closeAgendaModal');
  if (!openAgendaBtn || !agendaModal || !closeAgendaBtn) return;
...
// Intentar cargar el fragmento HTML del modal y después vincular eventos
fetch('modal-agenda.html').then(function (res) {
  if (!res.ok) throw new Error('No se pudo cargar modal-agenda.html');
  return res.text();
}).then(function (html) {
  const wrapper = document.createElement('div');
  wrapper.innerHTML = html.trim();
  // Añadir el primer elemento del fragmento al body
  if (wrapper.firstElementChild) document.body.appendChild(wrapper.firstElementChild);
  setupAgendaModalBindings();
}).catch(function (err) {
  console.warn('No se cargó modal-agenda.html — comprueba el servidor/paths:', err);
  // Si por alguna razón el modal quedó inline, aún intentamos enlazarlo
  setupAgendaModalBindings();
});
